...
public:
    int countBeautifulPairs(vector<int>& nums) {
        ...
        vector<int> cnt(10, 0);
        for (int y : nums) {
            // calculate the x number until current y, only focus on the first digit of x in cnt[0~9]
            for (int i = 0;i < 10;i++) {
                if (cnt[i] && __gcd(i, y % 10) == 1)
                    ans += cnt[i];
            }
...
int main() {
    Solution s;
    for (vector<int> nums : vector<vector<int>>{ {2,5,1,4}, {11,21,12} }) {
