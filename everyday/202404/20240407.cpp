...
class Tree {
   public:
    ...
    string name;
    bool dead;
    Tree(string n) : name(n), dead(false) {}
};

class ThroneInheritance {
    map<string, Tree*> match;
    Tree* king;
   ...
   public:
    ...
    void birth(string parentName, string childName) {
        Tree* p = match[parentName];
        Tree* c = new Tree(childName);
        p->child.push_back(c);
        match[childName] = c;
    }
...
int main() {
    ...
    t.birth("bob", "alex");
    t.birth("bob", "asha");
    vector<string> ret = t.getInheritanceOrder();
    for (auto s : ret)
        cout << s << " ";
    cout << endl;
    t.death("bob");
    ret = t.getInheritanceOrder();
    for (auto s : ret)
        cout << s << " ";
    cout << endl;
    return 0;
