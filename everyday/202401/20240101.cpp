...
public:
    int minOperationsMaxProfit(vector<int>& customers, int boardingCost, int runningCost) {
        int n = customers.size(), wait = 0, profit = 0, maxPro = 0, ans = -1, i = 0;
        while (i < n || wait>0) {
            if (i < n) wait += customers[i];
            int boardNum = min(wait, 4);
            wait -= boardNum;
            profit += boardingCost * boardNum - runningCost;
            ++i;
            if (profit > maxPro) {
                maxPro = profit;
                ans = i;
...
int main() {
    ...
    cout << sol.minOperationsMaxProfit(customers, boardingCost, runningCost) << endl;
    customers = { 10,9,6 };
