...
public:
    int maxmiumScore(vector<int>& cards, int cnt) {
        ...
        int n=cards.size(), idx=n-1, sum=0;
        while(cnt--) sum+=cards[idx--];
        if(sum%2==0) return sum;
        for(int l=idx;l>-1;l--){
            for(int r=idx+1;r<n;r++){
                if((cards[l]^cards[r])&1){
                    return sum+=cards[l]-cards[r];
                }
            }
        }
        return 0;
...
int main(){
    Solution s;
    vector<vector<int>> cards = {{1,2,8,9}, {3,3,1}};
    vector<int> cnt = {3, 1};
    for(int i=0;i<cards.size();i++){
        cout<<s.maxmiumScore(cards[i], cnt[i])<<endl;
    }
    return 0;
}
