        ...
        for (int i = 1; i < m; i++) {
            for (int j = 0; j < n; j++) {
                for (int k = 0; k < n; k++) {
                    ...
                    for (int j_pre = j - 1; j_pre <= j + 1; j_pre++) {
                        for (int k_pre = k - 1; k_pre <= k + 1; k_pre++) {
                            if (j_pre >= 0 && j_pre < n && k_pre >= 0 && k_pre < n && dp[i - 1][j_pre][k_pre] != -1) {
                                dp[i][j][k] = max(dp[i][j][k], dp[i - 1][j_pre][k_pre] + val);
                            }
                        }
                    }
                }
