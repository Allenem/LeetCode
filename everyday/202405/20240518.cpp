...
divisors[i] 的 可整除性得分 等于满足 nums[j] 能被 divisors[i] 整除的下标 j
的数量。
...
整除。 因此，返回 divisors[2] ，它的可整除性得分最大。
...
示例 3：
    ...
    由于 divisors[0] 和 divisors[1]
的可整除性得分都是最大的，因此，我们返回数值最小的一个，即 divisors[0] 。
